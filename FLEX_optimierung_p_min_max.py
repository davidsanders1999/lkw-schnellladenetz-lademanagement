...
def modellierung_p_max_min(szenario):
    ...
    print(f"Starte Optimierung für Szenario: {szenario}")
    logging.info(f"Start Modellierung für Szenario: {szenario}")
    ...
    print(f"Anzahl zu ladender LKWs: {len(df_lkw_filtered)}")
    logging.info(f"Anzahl LKWs im Szenario: {len(df_lkw_filtered)}")
    
    bidirektional = False if szenario.split('_')[10] == 'M' else True
    logging.info(f"Bidirektional: {bidirektional}")
    ...
    # ======================================================
    for strategie in ladestrategien:
        print(f"Starte Optimierung für Strategie: {strategie}")
        logging.info(f"Optimierung für Strategie: {strategie}")
        ...
        if model.Status == GRB.OPTIMAL:
            ...
            logging.info(f"Optimale Lösung für {strategie}")
            ...
            sum_energie = sum(P[(i, t)].X for i in range(I) for t in range(t_in[i], t_out[i] + 1))
            dict_geladene_energie[strategie] = sum_energie
            print(f"Energie {strategie}: {sum_energie:.2f} kWh")
            print(f"Ladequote: {sum(list_volladungen)/len(list_volladungen):.2%}")
            logging.info(f"Geladene Energie ({strategie}): {sum_energie:.2f} kWh")
        else:
            print(f"Keine optimale Lösung für {strategie} gefunden.")
            logging.warning(f"Keine Lösung für {strategie} gefunden.")
    
    # ======================================================
    # Nachbereitung: Ergebnisse sichern
    # ======================================================
    energie_diff = dict_geladene_energie['p_max'] - dict_geladene_energie['p_min']
    print(f"Energiedifferenz (p_max - p_min): {energie_diff:.2f} kWh")
    logging.info(f"Energiedifferenz (p_max - p_min): {energie_diff:.2f} kWh")
    ...
    print(f"CSV gespeichert: lastgang_{szenario}.csv & lastgang_lkw_{szenario}.csv")
    logging.info(f"✅ Export abgeschlossen für Szenario {szenario}")

# ======================================================
# Hauptfunktion
# ======================================================
def main():
    print("Starte Hauptlauf für alle Szenarien...")
    logging.info("Hauptlauf gestartet")

    for szenario in config.list_szenarien:
        print(f"Verarbeite Szenario: {szenario}")
        logging.info(f"Starte Szenario: {szenario}")
        modellierung_p_max_min(szenario)

    print("✅ Alle Szenarien erfolgreich optimiert")
    logging.info("Alle Optimierungen abgeschlossen")
...
if __name__ == '__main__':
    ...
    laufzeit = end - start
    print(f"Gesamtlaufzeit: {laufzeit:.2f} Sekunden")
    logging.info(f"Gesamtlaufzeit: {laufzeit:.2f} Sekunden")
